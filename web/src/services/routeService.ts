...
export const routeService = {
  async list(query: RoutesQuery = {}): Promise<RouteListResponse> {
    ...
    return { ...result, total: query.savedOnly ? filteredItems.length : result.total, items: filteredItems }
  },

  async create(payload: RouteCreateRequest): Promise<ApiRoute> {
    return apiRequest<ApiRoute>('/routes', { method: 'POST', body: payload })
  },
  ...
  async update(routeId: number, payload: RouteUpdateRequest): Promise<ApiRoute> {
    return apiRequest<ApiRoute>(`/routes/${routeId}`, { method: 'PATCH', body: payload })
  },
