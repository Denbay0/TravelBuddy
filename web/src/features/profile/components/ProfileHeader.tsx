...
export function ProfileHeader({ profile, onEdit, onLogout, isLoggingOut = false }: ProfileHeaderProps) {
  return (
    <header className="card-surface flex flex-col gap-6 p-6 md:flex-row md:items-center md:justify-between">
      <div className="flex items-center gap-4">
        <img src={profile.avatarUrl} alt={`Аватар пользователя ${profile.name}`} className="h-20 w-20 rounded-2xl object-cover" />
        <div>
