  ...
  return (
    <article className="overflow-hidden rounded-3xl border border-white/70 bg-white/80 shadow-glow dark:border-white/10 dark:bg-white/5">
      ...
      <div className="space-y-4 px-5 py-4"><p className="text-sm leading-relaxed text-ink/80">{post.caption}</p>
        ...
        <div className="flex gap-2"><textarea value={comment} onChange={(event) => setComment(event.target.value)} rows={2} placeholder="Оставьте комментарий о маршруте" className="w-full resize-none rounded-xl border border-ink/15 bg-white px-3 py-2 text-sm outline-none" /><button onClick={() => { const next = comment.trim(); if (!next) return; onComment?.(post, next); setComment('') }} className="rounded-xl bg-ink px-3 py-2 text-white"><Send size={15} /></button></div>
      </div>
    </article>
  )
}
